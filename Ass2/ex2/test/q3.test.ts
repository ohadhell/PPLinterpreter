...
describe('Q3 Tests', () => {
     ...
     it('trnasform class-exp program in to proc-exp 2', () => {
        expect(bind(bind(parseL31(`(L3 (define pair (class (a b) ((first (lambda () a)) (second (lambda () b)) (sum (lambda () (+ a b)))))) (let ((p12 (pair 1 2)) (p34 (pair 3 4))) (if (> (p12 'first) (p34 'second)) #t #f)))`), L31ToL3),  x=>makeOk(unparseL31(x)))).is.satisfy(isFailure);
   });
