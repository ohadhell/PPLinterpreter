...
const parseGoodClassExp = (typeName: Sexp, varDecls: Sexp, bindings: Sexp): Result<ClassExp> =>{
    ...
    const vars = mapResult(parseVarDecl,varDecls); //getting all the variables declerations
    const methods = parseBindings(bindings);//getting all the methods (bindings)
    //creating the class expressions
...
export const parsedToClassExps = (p: Parsed): ClassExp[] => {
    return parsedToClassExp(p,[]);
}

const parsedToClassExp = (p: Parsed, classArr: ClassExp[]): ClassExp[] => 
...
const getClassFromDefine = (exp: DefineExp, classArr: ClassExp[]): ClassExp[] => {
    return parsedToClassExp(exp.val,classArr);
}
...
const programHelper = (exp: Exp, exps: Exp[], classArr: ClassExp[]): ClassExp[] => {
    const tmp = parsedToClassExp(exp,classArr);
    if(isEmpty(exps)){
        return tmp;
    }
    return programHelper(first(exps), rest(exps), tmp);
