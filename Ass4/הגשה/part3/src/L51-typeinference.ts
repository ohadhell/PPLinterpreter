...
export const makeTEnvFromClasses = (parsed: A.Parsed): E.TEnv => {
    const classExpArr = A.parsedToClassExps(parsed);
    const tenv = E.makeEmptyTEnv();
    if(isEmpty(classExpArr)) return tenv;
    const classTNames = classExpArr.map((c:A.ClassExp) => c.typeName.var);
    const classTExpArr = classExpArr.map((c:A.ClassExp) => A.classExpToClassTExp(c));
    return E.makeExtendTEnv(classTNames,classTExpArr,tenv);
}
...
export const typeofDefine = (exp: A.DefineExp, tenv: E.TEnv): Result<T.VoidTExp> => {
    ...
    const val = exp.val;
    const constraint = bind(typeofExp(val, tenv),(valTE) => checkEqualType(varTE, valTE, exp));
...
const typeofProgramExps = (exp: A.Exp, exps: A.Exp[], tenv: E.TEnv): Result<T.TExp> => {
    if(A.isDefineExp(exp)){//found a define expression so we add the variable to the extended Tenv
        const extTEnv = E.makeExtendTEnv([exp.var.var],[exp.var.texp],tenv);
        return isEmpty(exps)? typeofExp(exp,extTEnv):
        bind(typeofExp(exp,extTEnv),_=> typeofProgramExps(first(exps),rest(exps),extTEnv));//continue typeofExp of the rest
    }
    return isEmpty(exps)? typeofExp(exp,tenv):
    bind(typeofExp(exp,tenv),_=> typeofProgramExps(first(exps),rest(exps),tenv));//continue typeofExp of the rest
}
...
export const typeofSet = (exp: A.SetExp, tenv: E.TEnv): Result<T.VoidTExp> => {
    const varTERes:Result<T.TExp> = E.applyTEnv(tenv,exp.var.var);//getting the varRef Texp from env
    const valTERes:Result<T.TExp> = typeofExp(exp.val, tenv);//getting the value Texp
    const constraint = safe2((valTE:T.TExp,varTE:T.TExp) => checkEqualType(valTE,varTE,exp))(valTERes,varTERes);
    return bind(constraint,_=> makeOk(T.makeVoidTExp()));
};
...
export const typeofClass = (exp: A.ClassExp, tenv: E.TEnv): Result<T.TExp> => {
    const fieldsTE:T.TExp[] = exp.fields.map((x:A.VarDecl) => x.texp);
    const fieldsName = exp.fields.map((arg) => arg.var);
    const extTenv = E.makeExtendTEnv(fieldsName,fieldsTE,tenv);
    const methodsVals = R.map((b) => b.val, exp.methods);
    const varTEs = R.map((b) => b.var.texp, exp.methods);
    const constraints = zipWithResult((varTE, metVal) => bind(typeofExp(metVal, extTenv),
                                                           (metValTE: T.TExp) => checkEqualType(varTE, metValTE, exp)),
                                      varTEs, methodsVals);

    const typeTE = E.applyTEnv(getFirstTenv(tenv),exp.typeName.var);
    return safe2((tmp, expTE:T.TExp) => makeOk(T.makeProcTExp(fieldsTE,expTE)))(constraints,typeTE);
};
//getting the "first" env
const getFirstTenv = (tenv: E.TEnv): E.TEnv => {//the first env is the one that holds the classes
    if(E.isExtendTEnv(tenv)){
        if(E.isEmptyTEnv(tenv.tenv))
            return tenv;
        return getFirstTenv(tenv.tenv);
    }
    return tenv;
}
