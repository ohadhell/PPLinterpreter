...
// import {asyncWaterfallWithRetry2} from '../src/part2';
...
describe('2.1 (PromisedStore)', () => {
    ...
    it('getAll retrieves an array', async () => {
        ...
        await expect(store.get(['a','b','c'])).to.be.rejectedWith(MISSING_KEY)
    })
})
...
describe('2.4 (asyncWaterfallWithRetry)', () => {
    ...
    it('retries twice my one function', async () => {
        let attempt = 1
        const v = await asyncWaterfallWithRetry([async () => {
            if (attempt == 3)
                return 1
