...
import { makeEmptyTEnv } from "../imp/TEnv";
 ...
 * const parseClassExp = (fields: Sexp, methods: Sexp[]): Result<ClassExp> => {
    if (!isArray(fields) || !allT(isString, fields)) 
        return makeFailure('Malformed ValsDec expression');
    const vars = bind(mapResult(parseL31CExp, fields),
                (cexps: CExp[]) => makeOk(map(makeVarDecl,fields)));
    if(!isGoodBindings(methods[0])){    
        return makeFailure('Malformed bindings in "lambda" expression');
    }
    const check = map(a =>a[1][0] === 'lambda', methods[0]);//checking for correct format of lamdba expressions
    if(check.includes(false)) return makeFailure('Malformed methods expressions');
    const names = map(b => b[0],methods[0]);
    const lambdaResult = mapResult(binding => parseL31CExp(second(binding)),methods[0]);
    const bindingsResult = bind(lambdaResult, (vals: CExp[]) => makeOk(zipWith(makeBinding, names, vals)));
...
const parseGoodClassExp = (typeName: Sexp, varDecls: Sexp, bindings: Sexp): Result<ClassExp> =>{
    if(!isArray(varDecls)) return makeFailure("Malformed valDecls");
    if(!isGoodBindings(bindings)) return makeFailure("Malformed bindings");
    const vars = bind(mapResult(parseVarDecl,varDecls),(x:VarDecl[]) =>  makeOk(x));
    const methods = parseBindings(bindings);
    if(!isString(typeName)) return makeFailure("Malformed typeName");
    return safe2((fields:VarDecl[],met:Binding[]) => makeOk(makeClassExp(makeTVar(typeName),fields,met)))(vars,methods);
}
...
// TODO parsedToClassExp
    return parsedToClassExpTry(p,[]);
}

const parsedToClassExpTry = (p: Parsed, classArr: ClassExp[]): ClassExp[] => 
isNumExp(p) ? [] :
isBoolExp(p) ? [] :
isStrExp(p) ? [] :
isPrimOp(p) ? [] :
...
isProcExp(p) ? [] :
isAppExp(p) ? [] :
isLetExp(p) ? [] ://maybe need to do something
isLetrecExp(p) ? [] ://maybe need to do something
isDefineExp(p) ? getClassFromDefine(p, classArr) :
isProgram(p) ? getClassFromProgram(p, classArr) :
// L51
isClassExp(p) ? getClassExpFromClass(p,classArr) : 
isLitExp(p) ? [] :
isSetExp(p) ? [] ://maybe need to do something here
[];

const getClassExpFromClass = (exp: ClassExp, classArr: ClassExp[]): ClassExp[] => {
    if(classArr.includes(exp)) return classArr;
    return classArr.concat(exp);
}

const getClassFromDefine = (exp: DefineExp, classArr: ClassExp[]): ClassExp[] => {
    if(!isClassExp(exp.val)) return classArr;
    if(classArr.includes(exp.val)) return classArr;
    return classArr.concat(exp.val);
}

const getClassFromProgram = (exp: Program, classArr: ClassExp[]): ClassExp[] => {
    return isEmpty(exp.exps) ? classArr :
    programHelper(first(exp.exps), rest(exp.exps), classArr);
}

const programHelper = (exp: Exp, exps: Exp[], classArr: ClassExp[]): ClassExp[] => {
    const tmp = parsedToClassExpTry(exp,classArr);
    if(isEmpty(exps)){
        if(tmp.length == classArr.length) return classArr; 
        return classArr.concat(tmp);
    }
    if(isEmpty(tmp)) return programHelper(first(exps), rest(exps), classArr);
    if(tmp.length == classArr.length) return programHelper(first(exps), rest(exps), tmp); 
    return classArr.concat(programHelper(first(exps), rest(exps), classArr.concat(tmp)));
