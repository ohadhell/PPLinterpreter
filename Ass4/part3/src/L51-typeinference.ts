...
export const typeofSet = (exp: A.SetExp, tenv: E.TEnv): Result<T.VoidTExp> => {
    const varTERes:Result<T.TExp> = E.applyTEnv(tenv,exp.var.var);
    const valTERes:Result<T.TExp> = typeofExp(exp.val, tenv);
...
export const typeofClass = (exp: A.ClassExp, tenv: E.TEnv): Result<T.TExp> => {
    ...
    // const extTenv:E.ExtendTEnv = E.makeExtendTEnv(exp.fields.map((x:A.VarDecl) => x.var),fieldsTE,tenv);
    // const methods:A.Binding[] = exp.methods;
    const constraint = mapResult((x: A.Binding) => bind(typeofExp(x.val,extTenv),(methodsTE: T.TExp) => checkEqualType(methodsTE,x.var.texp,exp)),exp.methods);
    // const methodsTEEesult = mapResult((x: A.Binding) => typeofExp(x.val,extTenv),methods);
    if (E.isEmptyTEnv(tenv)) return makeFailure("");
    const tmp = E.applyTEnv(tenv,exp.typeName.var);//tmp.value == ClassTexp
    return bind(constraint, _=> makeOk(T.makeProcTExp(fieldsTE,A.classExpToClassTExp(exp))));
};
