    ...
    describe('typeOfExp', () => {
        ...
        it('infers the type of a class constructor', () => {
            ...
            `(L5 (define pair (class : Tpair 
                 ...
                 (pair 1 2)
                 pair)`;
            expect(verifyTeOfExprWithInference(program1, `(number * number -> (class Tpair (first : (Empty -> number)) (rest : (Empty -> number))))`)).to.deep.equal(makeOk(true));
