...
import { applyEnv, makeExtEnv, Env, Store, setStore, extendStore, ExtEnv, applyEnvStore, theGlobalEnv, globalEnvAddBinding, applyLastAdded,theStore } from "./L21-env-store";
...
const applyClosure = (proc: Closure, args: Value[]): Result<Value> => {
    ...
    const addresses: number[] = map((x:Value) => applyLastAdded(extendStore(theStore, x)),args);
...
(rhs: Value) => { extendStore(theStore,rhs)
                    globalEnvAddBinding(def.var.var, applyLastAdded(theStore));
...
const evalLet = (exp: LetExp, env: Env): Result<Value> => {
    ...
    return bind(vals, (vals: Value[]) => {
        const addresses = map((v:Value) => applyLastAdded(extendStore(theStore,v)), vals);
...
const evalSet = (exp: SetExp, env: Env): Result<void> => {
    return safe2((val: Value, adrr: number) => makeOk(setStore(theStore, adrr, val)))
